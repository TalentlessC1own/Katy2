...
bool CheckTrapezoid(std::vector <double> trapezoidParts)
{
	{
		double sum = 0.0f;

		for (int i = 0; i < 4; i++) sum += trapezoidParts[i];

		for (int i = 0; i < 4; i++) if (trapezoidParts[i] >= sum - trapezoidParts[i]) return false;

		if (fabs(trapezoidParts[1] - trapezoidParts[0]) <= 0) return false;
